...
class TexturedPusherEnv7DOFExp2(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        utils.EzPickle.__init__(self)
        # mujoco_env.MujocoEnv.__init__(self, '/home/larry/dev/data-collect/examples/textured.xml', 5)
        mujoco_env.MujocoEnv.__init__(self, 'texture_7dof_push.xml', 5)
    ...
    def viewer_setup(self):
        cam_pos = np.array([0.4, -0.15, -0.1, 0.7, -50., 90.])
    ...
    def _get_viewer(self):
        ...
        if self.viewer is None:
            self.viewer = mujoco_py.MjViewer(init_width=80, init_height=60)
            self.viewer.start()
