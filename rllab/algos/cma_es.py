...
class CMAES(RLAlgorithm, Serializable):
    ...
    def train(self):
        ...
        while itr < self.n_itr and not es.stop():
            ...
            bestidx = np.argmax(undiscounted_returns)
            bestparams = xs[bestidx]
            self.policy.set_param_values(bestparams)
            evalpaths = []
            for j in range(10):
                path = rollout(
                    self.env,
            ...
            undiscounted_true_eval = np.array(
                [sum(path["env_infos"]["reward_true"]) for path in evalpaths])
            undiscounted_eval = np.array(
                [sum(path['rewards']) for path in evalpaths])
            trues = [sum(path["env_infos"]["reward_true"]) for path in infos]
            ...
            logger.record_tabular('EvalTrueAverageReturn',
                                  np.mean(undiscounted_true_eval))
            logger.record_tabular('EvalTrueStdReturn',
                                  np.std(undiscounted_true_eval))
            logger.record_tabular('EvalTrueMaxReturn',
                                  np.max(undiscounted_true_eval))
            logger.record_tabular('EvalTrueMinReturn',
                                  np.min(undiscounted_true_eval))

            logger.record_tabular('EvalAverageReturn',
                                  np.mean(undiscounted_eval))
            logger.record_tabular('EvalStdReturn',
                                  np.std(undiscounted_eval))
            logger.record_tabular('EvalMaxReturn',
                                  np.max(undiscounted_eval))
            logger.record_tabular('EvalMinReturn',
                                  np.min(undiscounted_eval))
