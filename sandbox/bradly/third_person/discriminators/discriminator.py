...
class ConvDiscriminator(Discriminator):
    ...
    def make_network(self, dim_input, dim_output):
        ...
        nn_input, time_input, target = self.get_input_layer(im_width, im_height, num_channels, dim_output)
        ...
        conv_out_flat = tf.concat(values=[tf.reshape(conv_layer_1, [-1, conv_out_size]), time_input], axis=1)
        ...
        self.class_target = target
        self.nn_input = [nn_input, time_input]
    ...
    def get_input_layer(im_width, im_height, num_channels, dim_output=2):
        ...
        time_input = tf.placeholder('float', [None, 1], name='time_input')
        targets = tf.placeholder('float', [None, dim_output], name='targets')
        return net_input, time_input, targets
...
class DomainConfusionVelocityDiscriminator(Discriminator):
    ...
    def make_network(self, dim_input, output_dim_class, output_dim_dom):
        ...
        conv_out_features_concat = tf.concat(axis=1, values=[conv_one_features_input_one, conv_one_features_input_two])
