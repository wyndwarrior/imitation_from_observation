...
class CyberPunkTrainer:
    ...
    def take_iteration(self, n_trajs_cost, n_trajs_policy):
        ...
        self.train_cost(training_data_one, training_data_two, training_classes, training_doms, n_epochs=10)
        ...
        logger.record_tabular('ReturnTrue', true_rew_mean)
    ...
    def train_cost(self, data_one, data_two, classes, domains, n_epochs):
        for iter_step in range(0, n_epochs):
            ...
            logger.record_tabular('GanLoss'+str(iter_step), np.mean(np.array(batch_losses)))
            logger.record_tabular('GanAcc'+str(iter_step), np.mean(np.array(lab_acc)))
